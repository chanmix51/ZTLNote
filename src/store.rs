...
mod tests {
    ...
    fn rename_path() {
        ...
        let metadata1 = store.add_note(topic, "main", "tmp/test10").unwrap();
        let metadata2 = store.add_note(topic, "main", "tmp/test10").unwrap();
        let path1 = "a_path";
        let new_path = "new_path";
        store.write_path(topic, path1, metadata2.note_id).unwrap();
        store.rename_path(topic, path1, new_path).unwrap();
        assert!(store.path_exists(topic, new_path));
        fs::remove_dir_all(base_dir).unwrap();
    }
}